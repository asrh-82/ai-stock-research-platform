...
def add_or_update_holding(portfolio, ticker, shares, avg_cost):
    ...
    portfolio.append({"ticker": ticker, "shares": round(float(shares), 8), "avg_cost": round(float(avg_cost), 8)})
    return portfolio, True
...
def remove_portfolio_holding(ticker_to_remove: str) -> None:
    ...
    portfolio = [holding for holding in portfolio if holding.get("ticker", "").strip().upper() != ticker_to_remove]
...
def get_portfolio_highlights(portfolio_df: pd.DataFrame):
    if portfolio_df.empty:
        return {"best": "N/A", "worst": "N/A", "largest": "N/A", "largest_sector": "N/A"}
